...
class ReputationProfiler {
  ...
  async analyzeAccount(accountId: string): Promise<{
    ...
    try {
      ...
      // Run parallel analysis with proper error handling
      const [domainAnalysis, espAnalysis, historyAnalysis] = await Promise.allSettled([
        this.analyzeDomain(account.email),
        this.detectESP(account),
        this.analyzeHistory(accountId),
      ])
      ...
      const domain = domainAnalysis.status === "fulfilled" 
        ? domainAnalysis.value 
        : this.getDefaultDomainAnalysis()
      
      const esp = espAnalysis.status === "fulfilled" 
        ? espAnalysis.value 
        : this.getDefaultESPAnalysis(account)
      
      const history = historyAnalysis.status === "fulfilled" 
        ? historyAnalysis.value 
        : this.getDefaultHistoryAnalysis()
      ...
      const reputationTier = this.determineReputationTier(riskScore, history)

      // Determine account role based on sending patterns
      const accountRole = this.determineAccountRole(history)
      ...
      // Determine warmup stage
      const warmupStage = this.determineWarmupStage(history)
      ...
      const profile = await prisma.reputationProfile.upsert({
        ...
        create: {
          ...
          hasSpfRecord: domain.hasSpf,
          hasDkimRecord: domain.hasDkim,
          hasDmarcRecord: domain.hasDmarc,
          espType: esp.type,
          espConfiguration: esp.config,
          totalEmailsSent: history.totalSent,
          lifetimeBounceRate: history.bounceRate,
          lifetimeOpenRate: history.openRate,
          lifetimeReplyRate: history.replyRate,
          lifetimeSpamRate: history.spamRate,
          riskScore,
          reputationTier,
          accountRole,
          recommendedDailyLimit,
          currentWarmupStage: warmupStage,
          daysInWarmup: history.daysInWarmup,
          industry: await this.detectIndustry(account.email),
          lastAnalyzedAt: new Date(),
          nextAnalysisAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days
        },
  ...
  /**
   * Get default domain analysis when DNS lookup fails
   */
  ...
  /**
   * Get default ESP analysis
   */
  private getDefaultESPAnalysis(account: any) {
    const email = account.email.toLowerCase()
    const domain = email.split("@")[1]
  ...
  /**
   * Analyze domain health and configuration with timeout
   */
  private async analyzeDomain(email: string): Promise<{
    domainAge: number | null
    reputation: number
    ...
    try {
      ...
      const timeout = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error("DNS lookup timeout")), this.DNS_TIMEOUT)
      })

      // Check DNS records in parallel with timeout
      const [spfRecords, dmarcRecords] = await Promise.race([
        Promise.all([
          dns.resolveTxt(domain).catch(() => []),
          dns.resolveTxt(`_dmarc.${domain}`).catch(() => []),
        ]),
      ...
      logger.warn("[ReputationProfiler] Domain analysis failed, using defaults", { 
        domain,
        error: error instanceof Error ? error.message : String(error)
      })
      
      return this.getDefaultDomainAnalysis()
    }
  }
  ...
  private async cacheProfile(accountId: string, profile: any): Promise<void> {
    try {
      ...
      logger.warn("[ReputationProfiler] Cache operation failed", { 
        accountId,
        error: error instanceof Error ? error.message : String(error)
      })
    }
  ...
  async getProfile(accountId: string): Promise<any> {
    try {
      ...
      logger.warn("[ReputationProfiler] Cache read failed", { accountId })
    }
  ...
  private async invalidateCache(accountId: string): Promise<void> {
    try {
      ...
      logger.warn("[ReputationProfiler] Cache invalidation failed", { accountId })
    }
  }
}
