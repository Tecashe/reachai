...
class RealtimeInboxManager {
  ...
  async notifyNewReply(replyId: string): Promise<void> {
    ...
    // Determine priority based on sentiment and category
    const { priority, playSound } = this.calculateReplyPriority(reply.sentiment, reply.category)
   ...
   * Used for initial load
   */
  async getRecentNotifications(userId: string, limit = 50): Promise<UnifiedNotification[]> {
    // Fetch from both models in parallel
    const [realtimeNotifs, standardNotifs] = await Promise.all([
      db.realtimeNotification.findMany({
        where: { userId },
   ...
   * Used for periodic polling
   */
  async pollNotifications(userId: string, lastChecked: Date): Promise<UnifiedNotification[]> {
    // Fetch new notifications from both models
    const [realtimeNotifs, standardNotifs] = await Promise.all([
      db.realtimeNotification.findMany({
        where: {
    ...
    // Merge and sort by creation date
    return [...realtimeUnified, ...standardUnified].sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime())
  }
   ...
   * Convert RealtimeNotification array to unified format
   */
  private convertRealtimeToUnified(notifications: any[]): UnifiedNotification[] {
    return notifications.map((n) => ({
      id: n.id,
      type: n.type,
      title: n.title,
   ...
   * Convert standard Notification array to unified format
   */
  private convertStandardToUnified(notifications: any[]): UnifiedNotification[] {
    return notifications.map((n) => ({
      id: n.id,
      type: n.type,
      title: n.title,
