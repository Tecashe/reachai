...
export async function getTemplates(
  ...
  try {
    ...
    const enhancedTemplates = templates.map((template) => ({
      ...template,
      avgClickRate: 0, // Default to 0 since it's not in the database yet
      totalSent: 0, // Default to 0 since it's not in the database yet
    }))
...
export async function duplicateTemplate(userId: string, templateId: string) {
  try {
    ...
    revalidatePath("/templates")
    return { success: true, template: enhancedDuplicate }
  } catch (err) {
