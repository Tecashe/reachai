...
//   } as Sequence & { steps: SequenceStep[] }
// }
...
// export async function updateSequence(sequenceId: string, userId: string, data: Partial<Sequence>): Promise<Sequence> {
//   // Remove fields that shouldn't be updated directly
//   const { id, createdAt, updatedAt, steps, ...updateData } = data as any
...
//   const { id, createdAt, updatedAt, variants, ...updateData } = data as any
...
//   sequenceId: string,
//   userId: string,
//   data: Partial<SequenceStepVariant>,
// ): Promise<SequenceStepVariant> {
//   // Verify ownership
//   const sequence = await db.sequence.findFirst({
//     where: { id: sequenceId, userId },
...
//   const winner = await db.sequenceStepVariant.findUnique({
//     where: { id: variantId },
//   })

//   if (!winner) throw new Error("Variant not found")

//   // Mark this variant as winner and reset others
...
//   // Update the main step content with winner's content
...
// export async function redistributeVariantWeights(stepId: string, sequenceId: string, userId: string): Promise<void> {
//   // Verify ownership
//   const sequence = await db.sequence.findFirst({
//     where: { id: sequenceId, userId },
//   })
...
//   const variants = await db.sequenceStepVariant.findMany({
//     where: { stepId },
//   })
...
// export async function getEnrollments(sequenceId: string, userId: string) {
//   // Verify ownership
//   const sequence = await db.sequence.findFirst({
//     where: { id: sequenceId, userId },
//   })
...
// export async function createEnrollment(sequenceId: string, userId: string, prospectEmail: string): Promise<any> {
//   // Verify ownership
//   const sequence = await db.sequence.findFirst({
//     where: { id: sequenceId, userId },
//   })
...
//   // Create a simple prospectId from email (in production, you'd look up or create a real prospect)
//   const prospectId = `prospect_${Buffer.from(prospectEmail).toString("base64").slice(0, 20)}`
...
// export async function getSequenceAnalytics(sequenceId: string, userId: string) {
//   // Verify ownership
//   const sequence = await db.sequence.findFirst({
//     where: { id: sequenceId, userId },
//     include: {
...
//   const totalBounced = sequence.steps.reduce((sum, s) => sum + s.bounced, 0)
...
// export async function getSequencePerformanceTrend(sequenceId: string, userId: string, days = 7) {
//   // Verify ownership
//   const sequence = await db.sequence.findFirst({
//     where: { id: sequenceId, userId },
//   })
...
//   const executions = await db.sequenceStepExecution.findMany({
//     where: {
//       step: {
//         sequenceId,
//       },
...
//   // Group by date
//   const dailyData: Record<string, { sent: number; opened: number; clicked: number; replied: number }> = {}

//   executions.forEach((exec) => {
//     if (exec.executedAt) {
//       const dateKey = exec.executedAt.toISOString().split("T")[0]
//       if (!dailyData[dateKey]) {
//         dailyData[dateKey] = { sent: 0, opened: 0, clicked: 0, replied: 0 }
//       }
//       dailyData[dateKey].sent++
//       if (exec.opened) dailyData[dateKey].opened++
//       if (exec.clicked) dailyData[dateKey].clicked++
//       if (exec.replied) dailyData[dateKey].replied++
//     }
//   })
...
//     seq.tags.forEach((tag) => allTags.add(tag))
//   })
...
//   const newTags = [...new Set([...sequence.tags, tag])]
...
//   const newTags = sequence.tags.filter((t) => t !== tag)
