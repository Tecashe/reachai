...
class ReplyDetector {
  async handleReply(event: ReplyEvent): Promise<void> {
    ...
    try {
      ...
      const campaign = emailLog.prospect.campaign
      if (campaign && !analysis.isAutoReply) {
        await emailNotificationService.sendReplyNotification({
          userId: campaign.userId,
          prospectEmail: fromEmail,
          prospectName: `${emailLog.prospect.firstName || ""} ${emailLog.prospect.lastName || ""}`.trim() || fromEmail,
          campaignName: campaign.name,
          replySubject: subject,
          replyPreview: body.substring(0, 150) + (body.length > 150 ? "..." : ""),
          sentiment: analysis.sentiment,
          category: analysis.category,
          replyUrl: `${process.env.NEXT_PUBLIC_APP_URL}/dashboard/inbox`,
        })
        ...
        await db.notification.create({
          data: {
            ...
            message: `${emailLog.prospect.firstName || fromEmail} replied to your email in ${campaign.name}`,
            entityType: "reply",
            entityId: reply.id,
