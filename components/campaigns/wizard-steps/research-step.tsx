...
export function ResearchStep({ campaign, onNext, onBack }: ResearchStepProps) {
  ...
  const handleSkipResearch = () => {
    toast.info("Skipping research - you can write emails manually")
    onNext()
  }
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-4 py-8">
        ...
        {!isResearching && researchedCount === 0 && (
          <div className="pt-4">
            <Button variant="outline" onClick={handleSkipResearch} className="gap-2 bg-transparent">
              Skip Research - Write Manually
            </Button>
            <p className="text-xs text-muted-foreground mt-2">
              You can write your own emails using variables like {"{"}firstName{"}"}, {"{"}company{"}"}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between pt-4 border-t">
        ...
        <Button
          ...
          className={!canProceed && researchedCount === 0 ? "invisible" : ""}
        >
          Continue to Email Generation
          <ArrowRight className="ml-2 h-4 w-4" />
        </Button>
