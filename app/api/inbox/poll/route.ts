...
// TEMPORARY: Return dummy data to save CPU until Jan 1
// TODO: Re-enable real notifications after Vercel usage resets
...
export async function GET(request: Request) {
  try {
    ...
    // Return dummy data immediately - no DB calls, no processing
    const dummyNotifications = [
      {
        id: "dummy-1",
        type: "reply",
        title: "Notifications temporarily paused",
        message: "Real-time inbox notifications will resume after Jan 1, 2025",
    ...
    console.log('[API Poll] Returning dummy data to conserve CPU')

    return NextResponse.json({ 
      notifications: dummyNotifications,
      nextPollAt: new Date(Date.now() + 12 * 60 * 60 * 1000).toISOString(),
      message: "Dummy data mode - notifications paused until Jan 1"
    })
  } catch (error) {
    console.error("[API Poll] Error:", error)
    return NextResponse.json({ error: "Failed to poll inbox" }, { status: 500 })
...
/* 
 * ORIGINAL CODE COMMENTED OUT FOR CPU SAVINGS
 * RESTORE AFTER JAN 1, 2025
 * 
import { db } from "@/lib/db"
import { realtimeInbox } from "@/lib/services/realtime-inbox"
