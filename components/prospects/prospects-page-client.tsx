...
import { getTrashedFolders, permanentlyDeleteFolder, restoreFolder } from "@/lib/actions/prospect-folders"
...
export function ProspectsPageClient({ initialFolders, initialTrashedCount }: ProspectsPageClientProps) {
  ...
  }, [showTrash, trashedFolders.length])
    ...
    <div className="space-y-6 p-6">
      ...
      {showFolderGrid && (
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div
              className="flex items-center justify-between p-6 border rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors cursor-pointer"
      ...
      {showTrash && (
        <div className="space-y-6">
          ...
          {trashedFolders.length > 0 && (
            <div>
              <h3 className="text-lg font-semibold mb-4">Deleted Folders</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {trashedFolders.map((folder) => (
                  <div key={folder.id} className="border rounded-lg p-4 bg-muted/50">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          if (result.success) {
                            toast.success("Folder restored!")
                            setTrashedFolders(trashedFolders.filter((f) => f.id !== folder.id))
                            window.location.reload()
                          } else {
                    ...
                    <p className="text-xs text-muted-foreground mb-3">{folder.prospectCount} prospects</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            <h3 className="text-lg font-semibold mb-4">Deleted Prospects</h3>
            <ProspectsList
              folderId={null}
              isTrashed={true}
              searchQuery={searchQuery}
