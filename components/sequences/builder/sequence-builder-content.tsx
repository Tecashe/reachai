...
const STATUS_CONFIG: Record<SequenceStatus, { label: string; className: string; dotClass: string }> = {
  DRAFT: {
    label: "Draft",
...
export function SequenceBuilderContent({ initialSequence, isNew = false, userId, campaignResearchInfo }: SequenceBuilderContentProps) {
  ...
  // Open mobile sheet when step selected on mobile
  React.useEffect(() => {
  ...
  return (
    <TooltipProvider>
      <div className="flex h-screen flex-col bg-background overflow-hidden">
        {/* Header */}
        <header className="sticky top-0 z-20 flex h-14 sm:h-16 shrink-0 items-center justify-between border-b border-border bg-card/95 backdrop-blur-sm px-3 sm:px-4 shadow-sm">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            {isEditingName ? (
              <div className="flex items-center gap-2">
                <Input
                  value={sequenceName}
                  onChange={(e) => handleNameChange(e.target.value)}
                  className="h-8 w-40 sm:w-64 text-sm sm:text-base"
                  autoFocus
                  onBlur={handleSaveName}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") handleSaveName()
              ...
              <button
                onClick={() => setIsEditingName(true)}
                className="group flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-muted transition-colors min-w-0"
              >
            ...
            <Badge variant="outline" className={cn("border gap-1.5 shrink-0 hidden sm:flex", statusConfig.className)}>
            ...
            {hasUnsavedChanges && (
              <Badge
                variant="outline"
                className="text-amber-600 border-amber-500/30 bg-amber-500/10 shrink-0 hidden sm:flex"
              >
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            {sequence.status === "DRAFT" || sequence.status === "PAUSED" ? (
              <Button
                size="sm"
                className="gap-1.5 h-8 sm:h-9 px-3 sm:px-4 shadow-sm"
                onClick={() => handleStatusChange("ACTIVE")}
                disabled={steps.length === 0}
              >
              ...
              <Button
                variant="outline"
                size="sm"
                className="gap-1.5 h-8 sm:h-9 px-3 sm:px-4 bg-transparent"
                onClick={() => handleStatusChange("PAUSED")}
              >
            ...
            <Button
              variant={hasUnsavedChanges ? "default" : "outline"}
              size="sm"
              className={cn("gap-1.5 h-8 sm:h-9 px-3 sm:px-4", !hasUnsavedChanges && "bg-transparent")}
              onClick={handleSave}
              disabled={isSaving}
            >
              {isSaving ? (
        ...
        {/* Tab bar */}
        <div className="sticky top-14 sm:top-16 z-10 flex h-12 sm:h-14 shrink-0 items-center justify-between border-b border-border bg-card/95 backdrop-blur-sm px-3 sm:px-4">
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
            <TabsList className="h-8 sm:h-9 bg-muted/50">
              <TabsTrigger value="builder" className="gap-1.5 text-xs sm:text-sm px-2 sm:px-3">
          ...
          {activeTab === "builder" && (
            <Button
              variant={activePanel === "settings" ? "secondary" : "outline"}
              size="sm"
              className="gap-1.5 h-8 sm:h-9 transition-colors bg-transparent"
              onClick={() => setActivePanel(activePanel === "settings" ? null : "settings")}
            >
              <Settings className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
              <span className="hidden sm:inline">Settings</span>
              <ChevronRight
                className={cn("h-3.5 w-3.5 transition-transform", activePanel === "settings" && "rotate-90")}
              />
            </Button>
          )}
        </div>
          ...
          {activeTab === "builder" && (
            <>
              ...
              {selectedStep && !isMobile && (
                <div className="w-[400px] xl:w-[440px] shrink-0 border-l border-border bg-card flex flex-col overflow-hidden">
                  <div className="flex-1 overflow-y-auto">
                    <SequenceStepPanel
                      step={selectedStep}
                      sequenceId={sequence.id}
                ...
                <div key={shortcut.action} className="flex items-center justify-between py-1">
                  ...
                  <div className="flex gap-1">
                    {shortcut.keys.map((key) => (
                      <kbd
                        key={key}
                        className="rounded-md border border-border bg-muted px-2 py-1 text-xs font-mono shadow-sm"
                      >
                        {key}
                      </kbd>
                    ))}
