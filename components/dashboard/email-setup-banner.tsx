...
export function EmailSetupBanner() {
  ...
  const checkEmailConnection = async () => {
    try {
      ...
      console.log("[v0] Email connection check:", data)

      // Check if data is an array and has items, or if it's an object with accounts property
      const accounts = Array.isArray(data) ? data : data.accounts || []
      setHasEmail(accounts.length > 0)
    } catch (error) {
      console.error("Failed to check email connection:", error)
    } finally {
      setLoading(false)
