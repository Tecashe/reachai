...
import { db } from "@/lib/db"
import { redirect } from "next/navigation"
import { RecheckButton } from "@/components/deliverability/recheck-button"
      ...
      {alerts.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Active Alerts</CardTitle>
            <CardDescription>
              Issues that need your attention. Click "Re-check" to verify if issues are resolved.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            {alerts.map((alert) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <RecheckButton domainId={alert.domainId} />
                  <Button variant="outline" size="sm" asChild>
                    <Link href={`/dashboard/email-setup?domain=${alert.domain}`}>Fix Now</Link>
                  </Button>
                </div>
                  ...
                  <div className="flex">
                    ...
                    <div className="flex-1">
                            ...
                            <CardDescription>
                              ...
                              {domain.lastHealthCheck && (
                                <span className="ml-2 text-xs">
                                  · Last checked: {new Date(domain.lastHealthCheck).toLocaleDateString()}
                                </span>
                              )}
                            </CardDescription>
                          </div>
                      ...
                      <CardContent className="space-y-4">
                        ...
                        {/* Actions */}
                        <div className="flex gap-2">
                          <RecheckButton domainId={domain.id} />
                          {!domain.isVerified && (
                            <Button asChild>
                              <Link href={`/dashboard/email-setup?domain=${domain.domain}`}>Configure DNS</Link>
                            </Button>
