...
import { EmailPreviewDialog } from "@/components/campaigns/email-preview-dialog"
import { CampaignManualSendButton } from "@/components/campaigns/campaign-manual-send-button"
import { SequenceTriggerConfig } from "@/components/campaigns/sequence-trigger-config"
import { getCampaignById, updateCampaignStatus } from "@/lib/actions/campaigns"
import { getProspects } from "@/lib/actions/prospects"
import { notFound } from 'next/navigation'
import Link from "next/link"
...
export default async function CampaignDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const sequenceTriggers = await db.subsequenceTrigger.findMany({
    where: { campaignId: id },
  }).catch(() => [])

  const availableSequences = await db.campaign.findMany({
    where: {
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="prospects" className="space-y-4">
        <TabsList>
          <TabsTrigger value="prospects">Prospects ({campaignProspects.length})</TabsTrigger>
          <TabsTrigger value="ab-testing">A/B Testing</TabsTrigger>
          <TabsTrigger value="sequence-triggers">Sequence Triggers</TabsTrigger>
          <TabsTrigger value="settings">Settings</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="sequence-triggers" className="space-y-4">
          <SequenceTriggerConfig 
            campaignId={id} 
            triggers={sequenceTriggers.map((t: {
              id: string;
              name: string;
              conditions: any;
              ...
              targetSequenceId: t.targetSequenceId || undefined,
