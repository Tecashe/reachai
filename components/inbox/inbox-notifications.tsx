...
// THE ABOVE ONE HAS THE POLLING WHILE THE BELOW ONE IS COMMENTED OUT
...
export function InboxNotifications() {
  ...
  const fetchNotifications = useCallback(async (isInitial = false) => {
    try {
      ...
      if (!response.ok) {
        // API is disabled, show message
        if (response.status === 503) {
          toast.info("Notifications temporarily paused", {
            description: "Will resume after Jan 1, 2025"
          })
        }
        throw new Error(`HTTP error! status: ${response.status}`)
      }
  ...
  // TEMP DISABLED: Polling paused to save CPU until Jan 1
  // useEffect(() => {
  //   const pollInterval = setInterval(() => {
  //     fetchNotifications(false)
  //   }, 10000)
  ...
  // (getPriorityConfig, getEntityConfig, formatTime, etc. - all unchanged)
