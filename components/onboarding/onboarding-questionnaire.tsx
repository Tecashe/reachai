...
//     console.log("[v0] Moving to step 5 (email connection) without submitting yet...")
//     setStep(5)
//     console.log("[v0] Step set to 5, should show email connection UI now")
//   }

//   const handleCompleteOnboarding = async () => {
//     setLoading(true)
//     console.log("[v0] Completing onboarding questionnaire...")
...
export function OnboardingQuestionnaire() {
  ...
  const logoSrc = currentTheme === "dark" ? "/mailfra-logo-dark.png" : "/mailfra-logo.png"
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    console.log("[v0] Saving onboarding data before moving to email connection...")
    
    // Save the questionnaire data first
    const result = await submitOnboardingQuestionnaire(formData)
    
    if (result.success) {
      console.log("[v0] Questionnaire saved, moving to step 5 (email connection)")
      setLoading(false)
      setStep(5)
    } else {
      toast.error(result.error || "Failed to save your information")
  ...
  const handleCompleteOnboarding = async () => {
    setLoading(true)
    console.log("[v0] Completing onboarding and redirecting to dashboard...")
    // Data already saved, just redirect
    toast.success("Welcome to mailfra!")
    router.push("/dashboard")
  }
