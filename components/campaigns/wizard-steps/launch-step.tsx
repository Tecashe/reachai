...
import { Input } from "@/components/ui/input"
import { Switch } from "@/components/ui/switch"
import { ArrowLeft, Rocket, Calendar, Clock, AlertCircle } from "lucide-react"
import { toast } from "sonner"
import { useRouter } from "next/navigation"
import { EmailConnectionRequired } from "@/components/campaigns/email-connection-required"
...
export function LaunchStep({ campaign, onBack }: LaunchStepProps) {
  ...
  const [isCheckingEmails, setIsCheckingEmails] = useState(true)
  const [settings, setSettings] = useState({
    startDate: new Date().toISOString().split("T")[0],
    dailyLimit: campaign.dailySendLimit || 50,
    sendInBusinessHours: true,
  ...
  useEffect(() => {
    const checkGeneratedEmails = async () => {
      try {
        const response = await fetch(`/api/campaigns/${campaign.id}/generated-emails`)
        if (response.ok) {
          const data = await response.json()
          setHasGeneratedEmails(data.hasGeneratedEmails)
        }
      } catch (error) {
        console.error("[v0] Failed to check generated emails:", error)
      } finally {
        setIsCheckingEmails(false)
      }
    }

    checkGeneratedEmails()
  }, [campaign.id])
  ...
  return (
    <div className="space-y-6">
      <EmailConnectionRequired />

      {!isCheckingEmails && hasGeneratedEmails && (
        <Alert variant="destructive">
          <AlertCircle className="h-4 w-4" />
          <AlertTitle>Note: Generated Emails Won't Be Sent</AlertTitle>
          <AlertDescription>
            You generated personalized emails during the campaign setup, but this launch uses the default campaign flow.
            If you want to use a custom sequence with your generated emails, please go back and save them as a sequence
            template first.
          </AlertDescription>
        </Alert>
      )}
