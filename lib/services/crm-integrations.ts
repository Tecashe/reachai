  ...
  async syncLeads(userId: string, apiKey: string): Promise<CrmLead[]> {
    try {
      ...
      do {
        ...
        url.searchParams.append("properties", "email,firstname,lastname,company,jobtitle,phone,linkedin_profile_url")
...
async function refreshHubSpotToken(
  ...
  try {
    ...
    if (!response.ok) {
      console.error("[builtbycashe] HubSpot token refresh failed:", await response.text())
      return null
    }
    ...
    console.error("[builtbycashe] HubSpot token refresh error:", error)
    return null
  }
}
...
export async function syncCrmLeads(
  ...
  try {
    console.log("[builtbycashe] Syncing CRM leads for user:", userId, "CRM type:", crmType, "Folder:", folderId)
    let leads: CrmLead[] = []
    ...
    if (crmType === "hubspot" && credentials.accessToken) {
      ...
      if (isExpired && credentials.refreshToken) {
        console.log("[builtbycashe] HubSpot token expired, refreshing...")
        ...
        if (newTokens) {
          ...
          activeCredentials.accessToken = newTokens.accessToken
          console.log("[builtbycashe] HubSpot token refreshed successfully")
        } else {
          console.error("[builtbycashe] Failed to refresh HubSpot token")
          return { imported: 0, skipped: 0, lowQualityContacts: [] }
        }
      }

      leads = await hubspotService.syncLeads(userId, activeCredentials.accessToken!)
      ...
      if (existingProspect) {
        ...
        await db.prospect.update({
          ...
          data: {
            ...
            crmData: lead as any,
            ...(folderId && !existingProspect.folderId ? { folderId } : {}),
          },
        })
        skipped++
      } else {
    ...
    console.log(
      `[builtbycashe] Synced ${imported} new leads, ${skipped} skipped, ${lowQualityContacts.length} low quality from CRM`,
    )
