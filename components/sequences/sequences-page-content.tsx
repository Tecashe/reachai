...
export function SequencesPageContent({ initialSequences, allTags, userId }: SequencesPageContentProps) {
  ...
  const [isCreating, setIsCreating] = React.useState(false)
  const [isBulkLoading, setIsBulkLoading] = React.useState(false)
  const [showAIGeneratorDialog, setShowAIGeneratorDialog] = React.useState(false)
              ...
              <div className="flex items-center gap-3">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-56">
                    <DropdownMenuItem onClick={() => setShowNewSequenceDialog(true)}>
        ...
        <div className="flex-1 overflow-auto p-6">
          {filteredSequences.length === 0 ? (
            <SequenceEmptyState
              hasSequences={sequences.length > 0}
              onCreateSequence={() => setShowNewSequenceDialog(true)}
              onGenerateWithAI={() => setShowAIGeneratorDialog(true)}
            />
          ) : view === "grid" ? (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filteredSequences.map((sequence) => (
