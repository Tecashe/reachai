...
export function OnboardingTour() {
  ...
  useEffect(() => {
    if (!document.getElementById('tour-pulse-animation')) {
      const style = document.createElement('style')
      style.id = 'tour-pulse-animation'
      style.textContent = `
        @keyframes tourPulse {
          0%, 100% {
    ...
    return () => {
      const style = document.getElementById('tour-pulse-animation')
  ...
  const openSidebarTemporarily = useCallback(() => {
    ...
    // Find the Sheet trigger button (your mobile sidebar)
    // First try: SheetTrigger with Menu icon inside
    let menuButton = document.querySelector('button:has(svg.lucide-menu)') as HTMLButtonElement
    
    // Fallback: Find button in header with Menu
    if (!menuButton) {
    ...
    if (menuButton) {
      menuButton.click()
      setSidebarOpened(true)
      console.log('[Tour] Opened mobile sidebar')
      
      // Wait for Sheet animation (300ms default for shadcn Sheet)
      setTimeout(() => {
        // Find the target element in the now-open sidebar
        const step = tourSteps[currentStep]
        const targetElement = document.querySelector(step.target) as HTMLElement
        
        if (targetElement) {
          // Highlight it with a pulsing effect
      ...
      console.warn('[Tour] Could not find mobile sidebar toggle button')
    }
  ...
  const closeSidebarIfOpened = useCallback(() => {
    ...
    // Find the Sheet close button or click outside overlay
    // shadcn Sheet has an overlay that closes on click
    const overlay = document.querySelector('[data-radix-overlay]') as HTMLElement
    
    if (overlay) {
      // Clear any pulse animation from highlighted element
      ...
      overlay.click()
      setSidebarOpened(false)
      console.log('[Tour] Closed mobile sidebar')
    } else {
      // Fallback: find the menu button and click it again to close
      const menuButton = document.querySelector('button:has(svg.lucide-menu)') as HTMLButtonElement
      if (menuButton) {
        menuButton.click()
        setSidebarOpened(false)
      }
  ...
  if (isMobile) {
    return (
      <>
        ...
        <div className="fixed inset-0 bg-black/80 z-[10000] backdrop-blur-sm pointer-events-none" />

        {/* Mobile centered modal - has pointer-events-auto */}
        <div className="fixed inset-0 z-[10001] flex items-center justify-center p-4 pointer-events-none">
          <div 
            className={cn(
              "pointer-events-auto w-full max-w-sm transition-all duration-300 ease-out",
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/70 z-[10000] pointer-events-none" />
