...
export function EmailGenerationStep({ campaign, onNext, onBack, isFirstStep }: EmailGenerationStepProps) {
  const [isGenerating, setIsGenerating] = useState(false)
  const [isLoadingProspects, setIsLoadingProspects] = useState(true)
  const [prospects, setProspects] = useState<ProspectWithEmail[]>([])
  const [generatedEmails, setGeneratedEmails] = useState<Record<string, GeneratedEmail>>({})
  ...
  useEffect(() => {
    const checkRateLimits = async () => {
      try {
        ...
        console.error("[v0] Failed to fetch rate limits:", error)
      } finally {
  ...
  const generateEmailsForAll = async () => {
    if (rateLimits && !rateLimits.allowed) {
      toast.error(`You've reached your ${rateLimits.tier} plan limit. Upgrade to continue.`)
      return
    }

    if (rateLimits && prospects.length > rateLimits.remaining) {
      toast.error(
        `You can only generate ${rateLimits.remaining} more emails this month. You have ${prospects.length} prospects.`,
      )
      return
    }
  ...
  return (
    <div className="space-y-6">
      {!isCheckingLimits && rateLimits && (
        <div
          className={`p-4 rounded-lg border ${
            rateLimits.remaining < 10 ? "bg-destructive/10 border-destructive" : "bg-accent/50 border-border"
          }`}
        >
          <div className="flex items-center justify-between">
            <div>
              <p className="font-semibold">AI Generation Credits</p>
              <p className="text-sm text-muted-foreground">
                {rateLimits.remaining} of {rateLimits.limit} remaining this month ({rateLimits.tier} plan)
              </p>
            </div>
            {rateLimits.remaining < 10 && <Badge variant="destructive">Low Credits</Badge>}
          </div>
        </div>
      )}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {/* Generate Button */}
          <Button
            onClick={useManualMode ? useManualTemplate : handleBulkGenerate}
            disabled={
              isGeneratingBulk ||
              generatedCount === prospects.length ||
              (rateLimits && rateLimits.remaining < prospects.length) ||
              undefined
            }
            className="w-full"
            size="lg"
          >
            {isGeneratingBulk ? (
              <>
                ...
                Generating {generatedCount}/{prospects.length}...
              </>
            ) : useManualMode ? (
              <>
                <Mail className="mr-2 h-4 w-4" />
      ...
      {/* Navigation */}
      <div className="flex items-center justify-between pt-4 border-t">
        <Button variant="outline" onClick={onBack} disabled={isGeneratingBulk}>
          <ArrowLeft className="mr-2 h-4 w-4" />
          Back
        </Button>
        <div className="flex gap-2">
          {/* Skip Button */}
          <Button variant="ghost" onClick={onNext} disabled={isGeneratingBulk}>
            Skip Generation
          </Button>
          <Button
            onClick={handleGenerateAndNext}
            disabled={isGeneratingBulk || (rateLimits && !rateLimits.allowed)}
            size="lg"
            className="gap-2"
          >
            <Sparkles className="h-4 w-4" />
            {isGeneratingBulk ? "Generating..." : "Generate & Continue"}
          </Button>
        </div>
      </div>
    </div>
