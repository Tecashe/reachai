...
/**
 * Manual Send API
 * Allows user to manually trigger IMMEDIATE email sending with strict limit enforcement
 */
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id: campaignId } = await params
    ...
    // Send allowed emails IMMEDIATELY instead of just queuing
    let sent = 0
    let failed = 0
      ...
      for (const schedule of schedules) {
        try {
          ...
          if (emailResult.success) {
            ...
            sent++
            console.log(`[v0] Sent email immediately to ${prospect.email}`)
          } else {
            await db.sendingSchedule.update({
              where: { id: schedule.id },
              data: {
          ...
          console.error(`[v0] Failed to send email immediately to ${prospect.email}:`, error)
          await db.sendingSchedule.update({
            where: { id: schedule.id },
            data: {
              status: "FAILED",
    ...
    return NextResponse.json({
      success: true,
      message: `${sent} emails sent immediately${failed > 0 ? `, ${failed} failed` : ""}${blocked.length > 0 ? `, ${blocked.length} blocked by company limits` : ""}`,
      sent,
      failed,
      blocked: blocked.length,
      blockedReasons: blocked.map((b) => ({ email: b.email, reason: b.reason })),
