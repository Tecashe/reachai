...
export default function GuidesPage() {
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      {/* Hero Section */}
      <section className="pt-20 pb-16 md:pt-32 md:pb-20 px-4 md:px-6 bg-background">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col items-center text-center mb-12 md:mb-16">
            <span className="text-xs md:text-sm font-medium text-muted-foreground tracking-wider uppercase mb-3 md:mb-4">Learning Center</span>
            <h1 className="text-3xl md:text-5xl lg:text-6xl font-bold text-foreground mb-4 md:mb-6">In-Depth Guides</h1>
            <p className="text-base md:text-lg text-muted-foreground max-w-2xl">
              Comprehensive, step-by-step guides to master every aspect of cold outreach. From beginner fundamentals to advanced strategies.
            </p>
          </div>

          {/* Featured Guide */}
          {featuredGuide && (
            <div className="relative rounded-2xl md:rounded-3xl overflow-hidden bg-card border border-border mb-16 md:mb-20">
              <Image
                src={featuredGuide.image || "/placeholder.svg"}
                alt={featuredGuide.title}
                fill
              ...
              />
              <div className="relative z-10 grid md:grid-cols-2 gap-6 md:gap-8 p-6 md:p-12 lg:p-16">
                <div className="flex flex-col justify-center">
                  <span className="inline-flex items-center px-3 py-1 bg-primary/10 rounded-full text-xs font-medium text-primary w-fit mb-4 md:mb-6">
                    Featured Guide
                  </span>
                  <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-foreground mb-3 md:mb-4">{featuredGuide.title}</h2>
                  <p className="text-muted-foreground text-base md:text-lg mb-6 md:mb-8">{featuredGuide.description}</p>
                  <div className="flex flex-wrap items-center gap-4 md:gap-6 mb-6 md:mb-8">
                    <div className="flex items-center gap-2 text-foreground">
                      <BookOpen className="w-4 h-4 md:w-5 md:h-5" />
                      <span className="text-sm md:text-base">{featuredGuide.chapters} Chapters</span>
                    </div>
                    <div className="flex items-center gap-2 text-foreground">
                      <Clock className="w-4 h-4 md:w-5 md:h-5" />
                      <span className="text-sm md:text-base">{featuredGuide.readTime} min</span>
                    </div>
                  </div>
                  <div className="flex flex-col sm:flex-row flex-wrap gap-3 md:gap-4">
                    <Link
                      href={`/guides/${featuredGuide.slug}`}
                      className="inline-flex items-center justify-center gap-2 px-6 md:px-8 py-3 md:py-4 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 transition-opacity"
                    >
                      Start Reading
                      <ArrowRight className="w-4 h-4" />
                    </Link>
                    <button className="inline-flex items-center justify-center gap-2 px-6 md:px-8 py-3 md:py-4 border border-border text-foreground rounded-lg font-medium hover:bg-card transition-colors">
                      <Download className="w-4 h-4" />
                      Download PDF
                    </button>
                  </div>
                ...
                {/* Table of Contents Preview */}
                <div className="hidden md:block">
                  <div className="bg-background/50 rounded-xl md:rounded-2xl p-4 md:p-6 border border-border">
                    <h3 className="text-foreground font-semibold mb-3 md:mb-4">Table of Contents</h3>
                      ...
                      {featuredGuide.tableOfContents.map((chapter, index) => (
                        ...
                        >
                          <div className="flex items-center gap-2 md:gap-3">
                            <span className="w-6 h-6 flex items-center justify-center bg-border rounded-full text-xs text-muted-foreground font-medium">
                              {index + 1}
                            </span>
                            <span className="text-foreground text-sm">{chapter.title}</span>
                          </div>
                          <span className="text-muted-foreground text-xs">{chapter.duration} min</span>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <section className="py-12 md:py-16 px-4 md:px-6 bg-background">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-8 md:mb-12">All Guides</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8">
            {otherGuides.map((guide, index) => (
              <div
                ...
                style={{ transitionDelay: `${index * 100}ms` }}
              >
                <div className="group relative bg-card border border-border rounded-xl md:rounded-2xl overflow-hidden hover:border-primary/50 hover:shadow-lg transition-all duration-300 h-full flex flex-col">
                  <div className="relative aspect-[16/9] overflow-hidden">
                    <Image
                      src={guide.image || "/placeholder.svg"}
                      alt={guide.title}
                    ...
                    <div className="absolute bottom-3 left-3 right-3 md:bottom-4 md:left-4 md:right-4">
                      <span className="px-2 md:px-3 py-1 bg-primary/90 rounded-md text-xs font-medium text-primary-foreground">
                        {guide.difficulty}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-4 md:p-6 flex flex-col flex-grow">
                    ...
                    <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6 text-sm text-muted-foreground">
                      <span className="flex items-center gap-1">
                        <BookOpen className="w-4 h-4" />
                        {guide.chapters} chapters
                      </span>
                    ...
                    <div
                      className={`overflow-hidden transition-all duration-300 ${
                        expandedGuide === guide.slug ? "max-h-96 mt-3 md:mt-4" : "max-h-0"
                      }`}
                    >
                      <div className="space-y-2 pt-3 md:pt-4 border-t border-border">
                        {guide.tableOfContents.slice(0, 5).map((chapter) => (
                          <div key={chapter.id} className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <Link
                      href={`/guides/${guide.slug}`}
                      className="mt-4 md:mt-6 flex items-center justify-center gap-2 w-full py-2 md:py-3 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 transition-opacity"
                    >
                      Start Guide
                      <ArrowRight className="w-4 h-4" />
                    </Link>
      ...
      {/* Request a Guide */}
      <section className="py-16 md:py-24 px-4 md:px-6 bg-card border-t border-border">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <button className="inline-flex items-center gap-2 px-6 md:px-8 py-3 md:py-4 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 transition-opacity">
            Request a Guide
            <ArrowRight className="w-4 h-4" />
          </button>
        </div>
