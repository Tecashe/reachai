...
const defaultPreviewImages = {
  platform: "/images/megamenu/platform.jpg",
  product: "/images/megamenu/product.jpg",
  resources: "/images/megamenu/resources.jpg",
  dashboard: "/images/megamenu/dashboard.jpg",
  company: "/images/megamenu/company.jpg",
}
...
export function MegaMenu({ label, sections }: MegaMenuProps) {
  ...
  // Get the currently hovered item with its preview data
  const getHoveredItemData = () => {
    if (!hoveredItemKey) {
      // Return first item from first section as default
      const firstItem = sections[0]?.items[0]
      if (firstItem) {
        return {
          item: firstItem,
          key: `${sections[0].title}-0`,
        }
      }
      return null
    }
    ...
    for (const section of sections) {
      for (let i = 0; i < section.items.length; i++) {
        if (`${section.title}-${i}` === hoveredItemKey) {
          return {
  ...
  return (
    ...
    >
      ...
      {/* Megamenu Panel - Desktop */}
      {isOpen && !isMobile && (
        <div className="absolute left-0 top-full bg-white dark:bg-slate-950 border border-border rounded-lg shadow-xl w-screen max-w-5xl z-50 mt-2">
          <div className="grid grid-cols-3 gap-0">
            {/* Left Column - Menu Items */}
            <div className="col-span-1 p-6 border-r border-border max-h-[550px] overflow-y-auto">
            ...
            {/* Right Column - Preview Image and Content */}
            {hoveredData && (
              <div className="col-span-2 p-8 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 flex flex-col justify-between">
                {hoveredData.item.image && (
                  <div className="relative w-full h-56 bg-muted rounded-lg overflow-hidden mb-6">
                    <Image
                      src={hoveredData.item.image || "/placeholder.svg"}
                      alt={hoveredData.item.title}
                      fill
                      className="object-cover"
                      priority
                    />
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className="text-3xl font-bold mb-3">{hoveredData.item.title}</h3>
                    <p className="text-base text-muted-foreground leading-relaxed">
                      {hoveredData.item.preview?.description || hoveredData.item.description}
                    </p>
                  </div>
                  {hoveredData.item.href && (
                    <div className="pt-2">
                      <Link href={hoveredData.item.href}>
                        <Button size="lg" className="gap-2">
                          {`Explore ${hoveredData.item.title}`}
    ...
    items: [
      {
        ...
        description: "Create and manage cold email campaigns",
        href: "/dashboard",
        image: "/images/megamenu/campaigns.jpg",
        preview: {
          title: "Campaigns",
          description: "Create, launch, and manage your cold email campaigns with advanced targeting, scheduling, and monitoring tools.",
        },
      },
      {
        icon: <BarChart3 className="w-5 h-5" />,
        ...
        description: "Real-time campaign performance tracking",
        href: "/dashboard",
        image: "/images/megamenu/analytics.jpg",
        preview: {
          title: "Analytics & Reports",
          description: "Track opens, clicks, replies, and conversions with detailed reports and real-time dashboards.",
        },
      },
    ],
  },
    ...
    items: [
      {
        ...
        description: "Manage leads and customer relationships",
        href: "/dashboard",
        image: "/images/megamenu/prospects.jpg",
        preview: {
          title: "Prospects & CRM",
          description: "Build and manage your prospect database with segmentation, enrichment, and CRM integration.",
        },
      },
      {
        icon: <GitBranch className="w-5 h-5" />,
        ...
        description: "Build automated follow-up sequences",
        href: "/dashboard",
        image: "/images/megamenu/sequences.jpg",
        preview: {
          title: "Sequences",
          description: "Design powerful automated follow-up sequences with conditional logic and timing controls.",
        },
      },
    ],
  },
    ...
    items: [
      {
        ...
        description: "Maximize inbox placement & deliverability",
        href: "/dashboard",
        image: "/images/megamenu/warmup.jpg",
        preview: {
          title: "Email Warmup",
          description: "Improve inbox placement rates with our intelligent email warmup engine and deliverability monitoring.",
        },
      },
      {
        icon: <Brain className="w-5 h-5" />,
        ...
        description: "AI-powered email writing and prediction",
        href: "/dashboard",
        image: "/images/megamenu/ai-tools.jpg",
        preview: {
          title: "AI Generation Tools",
          description: "Generate compelling subject lines and email body copy with AI. Predict reply rates before sending.",
        },
      },
    ],
  },
