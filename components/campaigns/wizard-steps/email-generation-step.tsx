    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Configure your email template for personalization</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Textarea
              id="body"
              placeholder={`Hi {firstName},\n\nI noticed {company} is working on [specific initiative]...\n\nWould you be open to a quick {callToAction}?\n\nBest,\n[Your name]`}
              value={template.body}
              onChange={(e) => setTemplate({ ...template, body: e.target.value })}
              rows={10}
              className="mt-2 font-mono text-sm"
            />
            <p className="text-xs text-muted-foreground mt-1">
              AI will use your research data to deeply personalize this
            </p>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Generate & Preview ({prospects.length} prospects)
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <div className="text-xs text-muted-foreground">
            {Object.keys(generatedEmails).length > 0 && (
              <p>
                ✓ {Object.keys(generatedEmails).length}/{prospects.length} emails generated
              </p>
            )}
          </div>
        </CardContent>
